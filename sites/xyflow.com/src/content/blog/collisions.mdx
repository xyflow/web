...
While exploring OpenAI’s Agent Builder and discovering they are preventing node overlaps,
obviously we got _nerd-sniped_ immediately.

[video]

You see, we’ve been keeping a long-running mental backlog of ideas and seeing one of them
...
elements is a fool’s errand in the real world. If you know, for example, that 95% of your
flows contain fewer than 25 nodes, your benchmark should reflect that reality.

The same goes for overlap density. In most cases, flows aren’t so tightly packed that
...
baseline cost — the performance you’d expect when there are no overlaps to resolve.
...
Unfortunately, we are just humble library authors with no access to production data… but
_maybe you_ can help us out?

If you’d like to see your real-world data included in our benchmarks, feel free to open a
pull request! It’s super easy, just add another array of trimmed down nodes
...
noticeable. We wouldn’t run this on every frame while dragging a node (which, on modern
...
- in a more realistic scenario of 500 clustered nodes, 1.1 ms seems perfectly acceptable
...
As a side note, this is pretty remarkable showcase of Apple Silicon’s single-core
performance. We should definitely run the same test on more representative machines in the
future to get a broader comparison.

### Optimize

If a single iteration (in our case, calculating the overlap) doesn’t seem very complex,
then the easiest way to optimize any \(O(n^2)\) algorithm is to bring that \(n\) down —
ideally to \(n \log n\), or, if we’re lucky, even \(\log n\).

Right now, our \(n\) comes from us comparing every single node with every other node like
the hairless ape we are. This, however, is a perfect use case for a

✨**Spatial Acceleration Structure**✨

While it might sound like technobabble straight out of _Mission: Impossible_, it’s really
just a clever data structure that lets us make cheap spatial queries such as: “Which nodes
lie within node X?”, which let's us get rid of those nested for-loops.

### quadtree-ts, rbush and flatbush enter the ring

Like the name implies, [quadtree-ts](https://github.com/timohausmann/quadtree-ts) is a
library utilizing quadtrees, which works by subdividing the space recursively and storing
only a limited number of nodes every leaf before further subdividing it.
...
however flatbush is entirely static (updating a single node means fully rebuilding tree)
and stores a its index in a single array buffer, which allows for passing around between
threads.

There is much to be studied with these libraries but let's keep it brief for this blog
post.
