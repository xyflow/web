...
In the [latest minor release](https://github.com/xyflow/xyflow/releases/tag/11.4.0), you can finally configure the controls so that they behave like Figma, Miro or Sketch. This includes click-dragging to make a selection, [pan by holding the space bar, and scrolling to zoom](https://reactflow.dev/learn/concepts/the-viewport). You can also configure if a node is selected if it is fully inside/partially inside the selection area.
...
**[Dynamic Grouping](https://reactflow.dev/examples/nodes/dynamic-grouping)** (Pro Example 💎): Grouping nodes made easy! Make new groups or add a node to an existing group by dragging a node from a drawer or using a “group nodes” button.
