...
This guide contains warnings and errors that can occur when using React Flow or Svelte
Flow. We also add common questions and pitfalls collected from our
...
<div id="001" />
### Warning: provider not found / could not find context.

<Tabs items={['React Flow', 'Svelte Flow']} storageKey="xyflow-library">
<Tabs.Tab>
...
**A:** You have two different versions of `@xyflow/react` installed.<br /> **B:** You are
trying to access the internal React Flow state outside of the React Flow context.

#### Solution for A

Update `@xyflow/react`, remove `node_modules` and your `package-lock.json` and reinstall
the dependencies.

#### Solution for B

A possible solution is to wrap your component with a
[`<ReactFlowProvider />`](https://reactflow.dev/api-reference/react-flow-provider) or move the code that is
...
Wrap your component with a [`<SvelteFlowProvider />`](https://svelteflow.dev/api-reference/svelte-flow-provider)
...
Or wrap with `<SvelteFlowProvider>` at the top level:
...
initial render. It can happen easily that you are defining the nodeTypes or edgeTypes
object inside of your component render function, which will cause React Flow to re-render
...
You don't have to dynamically update `nodeTypes` and `edgeTypes` — just create the object
once initially.
...
Make sure your wrapper component has a fixed height or inherits the height of its parent.
...
not have a parent node. You can remove the `extent` option or specify a `parentId`.
...
function Flow(props) {
  return <ReactFlow nodes={[]} edges={edges} />;
}
```
...
This can happen when you are trying to [reconnect an edge](https://reactflow.dev/examples/edges/reconnect-edge)
but the edge you want to update is already removed from the state. Please see the
[Reconnect Edge example](https://reactflow.dev/examples/edges/reconnect-edge) for
implementation details.

</Tabs.Tab>
<Tabs.Tab>

This can happen when you are trying to [reconnect an edge](https://svelteflow.dev/examples/edges/reconnect-edge)
but the edge you want to update is already removed from the state. Please see the
[Reconnect Edge example](https://svelteflow.dev/examples/edges/reconnect-edge) for
implementation details.
...
[updated the node internals after adding or removing handles](https://reactflow.dev/api-reference/hooks/use-update-node-internals)
programmatically. Please see the [Custom Node Example](https://reactflow.dev/examples/nodes/custom-node) for an
...
This can happen if you are working with multiple handles and a handle is not found by its
`id` property or if you haven't
[updated the node internals](https://svelteflow.dev/api-reference/hooks/use-update-node-internals) after adding
or removing handles programmatically. Please take a look at the
[documentation](https://svelteflow.dev/learn/customization/handles#using-multiple-handles) for an example on
how to use multiple handles.
...
[here](https://reactflow.dev/api-reference/types/edge#edgemarker).
...
[here](https://svelteflow.dev/api-reference/types/edge-marker).
...
component. This applies to both React Flow and Svelte Flow.
...
pass the correct `edgeTypes` property to React Flow. You can learn more about custom edges
in the [custom edge guide](https://reactflow.dev/learn/customization/custom-edges).
...
in the [custom edge guide](https://svelteflow.dev/learn/customization/custom-edges).
...
This can happen when a node is deleted before the `onNodeClick` handler fires. As the
warning says, this happens when you are trying to access a node that has already been
removed from the state. This applies to both React Flow and Svelte Flow.
...
Please import `@xyflow/react/dist/style.css` or `base.css` to make sure everything is
working properly.

This warning appears when you haven't imported the React Flow stylesheet. The styles are
...
custom node component. Call it inside a custom node or provide a node ID.

<div id="015" />
### It seems that you are trying to drag a node that is not initialized.

*(React Flow only)* This warning appears when you are trying to drag a node that has not
been initialized with the `onNodesChange` handler. Please make sure to use the
`onNodesChange` handler to initialize your nodes properly. You can learn more about this
in the ["Adding Interactivity" guide](https://reactflow.dev/learn/concepts/adding-interactivity).

### I get an error when building my app with webpack 4.

*(React Flow only)* If you're using webpack 4, you'll likely run into an error like this:
...
<Callout type="info">If you're using webpack 5, you don't need to do anything! React Flow will work out of the box.</Callout>
...
[`useReactFlow`](https://reactflow.dev/api-reference/hooks/use-react-flow) hook.
...
position by `1 / zoom` where `zoom` is the current zoom level of the flow. To get the
current zoom level, you can use the `getZoom` method from the
[`useSvelteFlow`](https://svelteflow.dev/api-reference/hooks/use-svelte-flow) hook.
...
- If you are using an async operation like a request to the backend, make sure to call the
  ...
  [`useUpdateNodeInternals`](https://reactflow.dev/api-reference/hooks/use-update-node-internals) hook after the
...
- If you are using an async operation like a request to the backend, make sure to call the
  ...
  [`useUpdateNodeInternals`](https://svelteflow.dev/api-reference/hooks/use-update-node-internals) hook after the async operation
...
- If you are changing the position of the handles (via reordering, etc.), make sure to
  ...
  [`useUpdateNodeInternals`](https://reactflow.dev/api-reference/hooks/use-update-node-internals) hook after so
...
- If you are using a custom edge and want your edge to go from the source handle to a
  ...
  [`getBezierPath`](https://reactflow.dev/api-reference/utils/get-bezier-path), etc.). `sourceX, sourceY`, and
...
- If the custom edge from the source or target side is not going towards the handle as
  ...
  [`getBezierPath`](https://reactflow.dev/api-reference/utils/get-bezier-path)). Passing the source/target
...
- If you are changing the position of the handles (via reordering, etc.), make sure to
  ...
  [`useSvelteFlow`](https://svelteflow.dev/api-reference/hooks/use-svelte-flow) hook after so Svelte Flow knows
...
- If you are using a custom edge and want your edge to go from the source handle to a
  ...
  function (e.g., [`getBezierPath`](https://svelteflow.dev/api-reference/utils/get-bezier-path), etc.).
...
- If the custom edge from the source or target side is not going towards the handle as
  ...
  `sourcePosition` and `targetPosition` props you get from the custom edge component in
  the edge path creation function (e.g.,
  [`getBezierPath`](https://svelteflow.dev/api-reference/utils/get-bezier-path)). Passing the source/target
  handle position in the path creation function is necessary for the edge to start or end
  properly at a handle.
