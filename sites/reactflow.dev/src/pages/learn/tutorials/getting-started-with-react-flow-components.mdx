...
export const getStaticProps = getStaticCode([
  ...
  'tutorials/components/num-node',
  'tutorials/components/complete',
]);
...
<Callout>
  ...
  have access to responsive media queries or pseudo-classes like `hover` and
  `focus`.
</Callout>
...
First let's remove the `<TooltipNode />` and undo our changes to `<BaseNode />`.
In addition to premade nodes, React Flow Components also contains building blocks
for creating your own custom nodes. To see them, we'll add the `node-header` and
`labeled-handle` components:
...
<Callout type="info">
  Notice how adding the `node-header` component also brought with it some
...
```tsx filename="src/components/nodes/num-node.tsx" {9-18,44-53,56,58,62,64}
...
<Callout>
  This isn't a tutorial for basic React Flow concepts like flows and custom
  nodes so we're **skipping over some of the basics**. If you're new to React
  Flow and want to learn how to add custom nodes and edges to a flow, check out
  the [guide on custom nodes](/learn/customization/custom-nodes).
</Callout>

In the snippet above we've highligted the imports and components that come from
...
To better visualise data in our calculator flow, let's pull in the `data-edge`
...
const initialEdges: Edge[] = [
  { id: 'a->c', type: 'data', data: { key: 'value' }, source: 'a', target: 'c', targetHandle: 'x' },
  { id: 'b->c', type: 'data', data: { key: 'value' }, source: 'b', target: 'c', targetHandle: 'y' },
  { id: 'c->e', type: 'data', data: { key: 'value' }, source: 'c', target: 'e', targetHandle: 'x' },
  { id: 'd->e', type: 'data', data: { key: 'value' }, source: 'd', target: 'e', targetHandle: 'y' },
];
...
- How to use building blocks like the [`<NodeHeader />`](/components/node-blocks/node-header)
