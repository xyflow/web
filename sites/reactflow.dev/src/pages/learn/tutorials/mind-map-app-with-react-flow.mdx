...
To be able to use React Flow hooks, we need to wrap the application with the [`ReactFlowProvider`](/api-reference/ReactFlowProvider) component in our main.tsx (entry file for vite). We are also importing the newly created `App/index.tsx` and render it inside the `ReactFlowProvider.` Your main file should look like this:
...
This function returns the position of the new node we want to add to our store. We are using the [`project` function](/api-reference/types/ReactFlowOnstance/#project) to convert screen coordinates into React Flow coordinates. As mentioned earlier, child nodes are positioned relative to their parents. That's why we need to subtract the parent position from the child node position. That was a lot to take in, let's see it in action:
...
We are passing all props to the [`getStraightPath`](/api-reference/utils/getStraightPath) helper function but adjust the sourceY so that it is in the center of the node.
