...
the number of folks using React Flow in more static contexts, such as visualising
...
for markdown slides, keyboard navigation and automatic layouting. To follow along
...
You can also take a look at a repo with the final code [here](https://github.com/xyflow/react-flow-slide-show)
...
right, up, or down. This part of the guide gets a bit nerdy, but stick with us!
...
```tsx filename="App.tsx" {4-8,14,22}
import { ReactFlow } from 'reactflow';
...
Because we have the idea of an "initial" slide now, we're also passing some options
that ensure the initial slide is the one that is focused when the canvas is first
loaded.
...
- Keyboard navigation using the arrow keys. This will allow us to move around
...
```tsx filename="App.tsx" {2,16,17-20,25}
...
<Callout>
  ...
  our `handleNodeClick` callback. That's because the `fitView` function is replaced
  once React Flow has initialised the viewport. If you forget this step you'll
  likely find out that `handleNodeClick` does nothing at all (and yes, we also
  forget this ourselves sometimes too <Emoji content="🫠" />).
</Callout>
...
```tsx filename="Slide.tsx" {3,6,11-16}
