---
title: 'Create a slide presentation with React Flow'
intro:
...
And once you're done, the app will look like this!
...
<Callout>
  ...
  styles! If you don't need the default styles, at a minimum you should include the
  base styles from `reactflow/dist/base.css`.
</Callout>
...
```tsx filename="App.tsx" {2,3,5-9,15,23}
import { ReactFlow } from 'reactflow';
...
```tsx filename="App.tsx" {1-2,17-21,26}
import { useCallback } from 'react';
import { ReactFlow, useReactFlow, type NodeMouseHandler } from 'reactflow';
import { Slide, SlideData, SLIDE_WIDTH } from './Slide';
...
[`FitViewOptions`](/api-reference/types/FitViewOptions) object lets us provide an
...
import { type NodeProps, fitView } from 'reactflow';
import { Remark } from 'react-remark';
import { useCallback } from 'react';
...
[`FitViewOptions`](/api-reference/types/FitViewOptions) we can see that the array
...
<Bleed>
  <BlogExampleViewer
    style={{ aspectRatio: '16 / 9' }}
    codePath="blog-flows/presentation/SlideControls"
    additionalFiles={['Slide.tsx', 'slides.ts', 'index.css']}
    dependencies={{ 'react-remark': '2.1.0' }}
    isTypescript
  />
