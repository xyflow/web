...
nodes. These are paths always SVG-based and are typically rendered using the
...
![](/img/learn/edge-label-renderer-position.png)
...
<Callout type="info">
  ...
  is important to add `pointer-evens: all` to the label's style. This will ensure
  that the label is clickable.

And just like with interactive controls in custom nodes, we need to remember
to add the `nodrag` and `nopan` classes to the label to stop mouse events also
