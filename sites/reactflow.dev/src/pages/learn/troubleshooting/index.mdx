...
- If the custom edge from the source or target side is not going towards the handle as expected, make sure to pass the `sourcePosition, targetPosition` props you get from the custom edge component in the edge path creation function(e.g [`getBezierPath`](https://reactflow.dev/api-reference/utils/get-bezier-path) etc). Passing the `source/target` handle position in the path creation function, is necessary for the edge, to properly start or end at a handle.