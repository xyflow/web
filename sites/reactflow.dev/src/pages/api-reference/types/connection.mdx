---
title: Connection
description:
  'The `Connection` type is the basic minimal description of an `Edge` between
  two nodes. The `addEdge` util can be used to upgrade a `Connection` to an `Edge`.'
---

import { PropsTable } from '@/components/props-table';
import { connectionFields } from '@/page-data/reference/types/Connection.fields.ts';

# Connection

[Source on GitHub](https://github.com/wbkd/react-flow/blob/bacaa99c73b347856f63c5d495f5a0829be1e431/packages/core/src/types/general.ts/#L57)

The `Connection` type is the basic minimal description of an [`Edge`](/api-reference/types/edge)
between two nodes. The [`addEdge`](/api-reference/utils/add-edge) util can be used to upgrade
a `Connection` to an [`Edge`](/api-reference/types/edge).

```ts
export type Connection = {
  source: string | null;
  target: string | null;
  sourceHandle: string | null;
  targetHandle: string | null;
};
```

## Fields

<PropsTable {...connectionFields} />
