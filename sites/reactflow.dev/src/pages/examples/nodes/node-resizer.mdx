...
<ExampleLayout>
The [`<NodeResizer />`](/api-reference/components/node-resizer) component can be used to add a resize UI for a custom node. The `node-resizer` package also exports a `<NodeResizeControl />` component for implementing a custom resizing UI as shown in this example.
