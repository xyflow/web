...
<ExampleViewer
  codePath="example-flows/ConnectionEvents"
...
- [`onConnect`](/api-reference/react-flow#on-connect) is only called when the
  connection is released on a handle that [is connectable](/components/handle#is-connectable).
...
<Callout type="info">
  You can see many of these events in use in our [add node on edge drop](/examples/nodes/add-node-on-edge-drop)
  and [temporary edges](/examples/edges/temporary-edges) examples!
</Callout>
