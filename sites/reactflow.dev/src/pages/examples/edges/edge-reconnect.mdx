...
<ExampleLayout>
You can reconnect an edge by dragging it to another handle if you are using the
[`onReconnect` handler prop](/api-reference/react-flow#onReconnect). The handler gets
...
- Individual edges can refine or override the [`edgesUpdatable`](/api-reference/react-flow#edgesReconnectable)
  prop by setting their [`updatable`](/api-reference/types/edge#reconnectable) property.
