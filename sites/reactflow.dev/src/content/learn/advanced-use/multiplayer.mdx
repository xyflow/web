...
This is only true if mergeMap merge operation is synchronous and react render timing doesn't 
come in between (interleaved) user input and remote subscription never lags behind local optimistic updates.
This depends on the solution used.
In real React the user drags, local state mutates, remote echo comes back out-of-order
and  mergeMap overwrites with derived state.

We are assuming perfect causality here.
*/}
