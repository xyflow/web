...
You can add a new node type to React Flow by adding it to the `nodeTypes` prop. **`nodeTypes` must be memoized or defined outside of the component.** Otherwise, React creates a new object on every render, which leads to performance issues and bugs.
...
Applying these utility classes helps you control interaction on a granular level. You can custimize these class names inside React Flow's [style props](/api-reference/react-flow/#style-props).
