...
We have a ready-to-use [Vite template](https://github.com/xyflow/vite-react-flow-template)
that you can use to get up and running in no time.

```bash copy npm2yarn
npx degit xyflow/vite-react-flow-template app-name
```

### Vite template with React

To create a new React Flow project from scratch, first, spin up a new React project
however you like -- we recommend using [Vite](https://vitejs.dev/)
