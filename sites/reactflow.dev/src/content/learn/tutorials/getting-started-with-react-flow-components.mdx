...
[`<BaseNode />`](/components/nodes/base-node) component with a single command:
...
Remember that the `<BaseNode />` component is used by any _other_ React Flow components we
...
addition to pre-made nodes, React Flow Components also contains building blocks for
creating your own custom nodes. To see them, we'll add the `labeled-handle` component:
...
React Flow Components doesn't just provide components for building nodes. We also provide
...
- How editing the [`<BaseNode />`](/components/nodes/base-node) component will affect
  other nodes pulled from the React Flow Components registry.

- How to use building blocks like the
  [`<BaseNodeHeader />`](/components/node-blocks/base-node) and
  [`<LabeledHandle />`](components/handles/labeled-handle) components to build our own
  custom nodes without starting from scratch.

- That React Flow Components also provides custom edges like the
  [`<DataEdge />`](/components/edges/data-edge) to drop into our applications.
...
[components docs page](/components). The React Flow Components project is still in its
infancy: if you have any suggestions or requests for new components we'd love to hear
about them. Or perhaps you're already starting to build something with shadcn and React
Flow Components. Either way make sure you let us know on our
