...
- How editing the [`<BaseNode />`](/ui/components/base-node) component will affect other
  nodes pulled from the React Flow Components registry.
...
[UI docs page](/ui). The React Flow Components project is still in its infancy: if you
have any suggestions or requests for new components we'd love to hear about them. Or
perhaps you're already starting to build something with shadcn and React Flow Components.
Either way make sure you let us know on our
