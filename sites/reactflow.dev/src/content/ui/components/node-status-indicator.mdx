...
- The `"border"` variant is the default and shows a spinning border around the node when
  it is in loading state.
- The `"overlay"` variant shows a full overlay, with an animated spinner on the node when
  it is in loading state.
