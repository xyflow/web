...
export type WideImageProps = {
  src: string;
  alt: string;
  caption?: string;
};

export function WideImage({ src, alt, caption }: ImagPropse) {
  return (
    <figure className={`relative h-full my-8 mx-0 ${negativeMargin}`}>
      <NxImage
        src={src}
        alt={alt}
...
export function Embed({ src }: EmbedProps) {
  return (
    <div
      className={`relative h-full my-8 mx-0 ${negativeMargin} rounded-xl bg-gray-50`}
    >
      <iframe src={src} className="h-[50vh] w-full" />
    </div>
  );
}
