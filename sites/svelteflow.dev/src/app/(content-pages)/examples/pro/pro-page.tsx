...
import { ListWrapper } from 'xy-shared/components/ui/list-wrapper';
import { cn } from 'xy-shared/lib/utils';
import type { HeroIcon } from 'xy-shared/types';
...
import { LayoutBreakout } from 'xy-shared/layouts/breakout';
import { fetchJSON } from 'xy-shared/lib/fetch-json';
import {
  ArrowDownCircleIcon,
...
const ProExamples: FC = async () => {
  const remoteProExamplesResponse = await fetchJSON(
    `${process.env.NEXT_PUBLIC_PRO_EXAMPLES_URL}/${PRO_EXAMPLES_FRAMEWORK}/apps.json?t=1`,
  );
  ...
  >((result, curr) => {
    ...
    if (remote) {
      result.push({
        ...remote,
        route: curr.route,
        image: `${process.env.NEXT_PUBLIC_PRO_EXAMPLES_URL}/${PRO_EXAMPLES_FRAMEWORK}/${remote.id}/thumbnail.jpg`,
      });
    }
