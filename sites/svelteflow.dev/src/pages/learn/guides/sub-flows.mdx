...
To define a node as a child of another node, you need to use the `parentNode` option (you can find a list of all options in the [node options section](/api-reference/types/node)). Once we do that, the child node is positioned relative to its parent. A position of `{ x: 0, y: 0 }` is at the top left corner of the parent.
