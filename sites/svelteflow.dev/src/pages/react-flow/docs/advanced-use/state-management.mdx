...
<Callout type="info">
  ...
  concepts](/docs/concepts/core-concepts/) of React Flow and how to implement
  [custom nodes](/docs/guides/custom-nodes/). You should also be familiar with
  the concepts of state management libraries and how to use them.
</Callout>
...
<CodeViewer
  ...
  additionalFiles={['store.ts', 'nodes.ts', 'edges.ts']}
  isTypescript
/>
...
<CodeViewer
  ...
  applyStyles={false}
  options={{ editorHeight: 500, editorWidthPercentage: 45, wrapContent: true }}
  additionalFiles={['ColorChooserNode.tsx', 'store.ts', 'nodes.ts', 'edges.ts']}
  isTypescript
/>
