...
The `useSvelteFlow` hook exports helpers to check intersections of nodes and areas. In this example you can drag a node and get a visual feedback when it intersects with another node.
  ...
  <RemoteCodeViewer
    source={`${process.env.NEXT_PUBLIC_EXAMPLES_URL}/svelte/examples/nodes/intersections/source.json`}
    preview={`${process.env.NEXT_PUBLIC_EXAMPLES_URL}/svelte/examples/nodes/intersections/index.html`}
    framework="svelte"
  />
</ExampleLayout>
