---
description:
  Learn how to create and implement custom nodes in Svelte Flow. This guide covers node
  component creation, handle management, positioning, and utility classes for building
  interactive and flexible flow diagrams.
---
...
A powerful feature of Svelte Flow is the ability to create custom nodes. This gives you
the flexibility to render anything you want within your nodes. We generally recommend
creating your own custom nodes rather than relying on built-in ones. With custom nodes,
you can add as many source and target handles as you like—or even embed form inputs,
...
To create a custom node, all you need to do is create a Svelte component. Svelte Flow will
...
connecting handles. For a full reference on all available custom node props, take a look
at the [Node Props](/api-reference/types/node-props).
...
[`updateNodeData`](/api-reference/hooks/use-svelte-flow#update-node-data) function, that
can be accessed through the [`useSvelteFlow`](/api-reference/hooks/use-svelte-flow) hook.
...
passing the [`nodeTypes`](/api-reference/svelte-flow#node-types) prop.
...
After putting it all together and adding some basic styles we get a custom node that
...
Svelte Flow provides a [`Handle`](/api-reference/components/handle) component that can be
...
[`useUpdateNodeInternals`](/api-reference/hooks/use-update-node-internals) context
...
interactions work within your custom nodes.
...
customize these class names through Svelte Flow
