...
- **elevateNodesOnSelect** prop: Control if nodes should be elevated via z-index when selected.
...
<Callout>
  You have to use `$derived(useSvelteFlow());` if you are using the hook outside of the
  main `<SvelteFlow />` component via the `<SvelteFlowProvider />
  `.
</Callout>
