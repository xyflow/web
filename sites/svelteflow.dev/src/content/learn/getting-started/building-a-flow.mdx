...
<Callout type="info">
  Any content rendered inside the `SvelteFlow` component will be overlaid on top of the
  viewport. This is perfect for UI elements that should remain fixed and not move with the
  viewport. The Background component is fixed but
</Callout>
...
We'll use the [`$state`](https://svelte.dev/docs/svelte/$state) rune to make the array reactive.

<Callout color="yellow">
  Simply using `$state` would not only make the array reactive, but every property of
  eachnode object, too. This could lead to [performance
...
Let's connect the nodes with an edge. Create a [`$state.raw`](https://svelte.dev/docs/svelte/$state#$state.raw) array with [edge objects](/api-reference/types/edge) that have these required properties:
...
Finally, let's enhance the edge by changing its type to `smoothstep` and adding a label:
