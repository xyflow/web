...
A couple of properties interact with one another to determine whether an edge is updatable
or not:
...
- Individual edges can refine or override the
  ...
  their [`updatable`](/api-reference/types/edge#updatable) property.
  - `true` means the edge is updatable even if
    [`edgesReconnectable`](/api-reference/react-flow#edgesreconnectable)

  - `"source"` or `"target"` means the edge is updatable only from the source or target
