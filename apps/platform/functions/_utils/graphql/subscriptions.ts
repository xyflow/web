...
async function upsertSubscription({
  ...
  return await GraphQLClient.request<UpsertSubscriptionResponse>(UPSERT_SUBSCRIPTION, {
    userId,
    planId,
    stripeCustomerId,
  });
...
export async function handleSubscriptionChange(stripeEvent: Stripe.Subscription) {
  ...
  if (customer && customer.email) {
    ...
    if (planId && userId && status === 'active') {
      await upsertSubscription({ userId, planId, stripeCustomerId: customerId });
    }

    if (userId && (status === 'past_due' || status === 'canceled')) {
      await upsertSubscription({ userId, stripeCustomerId: customerId, planId: 'free' });
    }
  }
}
