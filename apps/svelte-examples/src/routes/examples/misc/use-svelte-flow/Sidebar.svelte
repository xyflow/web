...
<aside>
  ...
  <button on:click={() => zoomOut({ duration: 1000 })}
  ...
  <button on:click={() => setViewport({ x: 100, y: 100, zoom: 2 })}
    >setViewport</button
  >
  ...
  <div>
    x: {$viewport.x.toFixed(1)} y: {$viewport.y.toFixed(1)} zoom: {$viewport.zoom.toFixed(
      1
    )}
  </div>
</aside>
